...
7. **학생 유행을 짜는 어른(영양사)이 따라가는가? — '메뉴화 가능한 것'만.** 유행식(마라·약과·
   ...
   정체(28→30)·건강식은 2024 정점 후 하락(11.8→10.3). 키워드는 FastText 임베딩으로 추출하되 **5라운드
   적대검증** + 데이터 재현·부트스트랩·외부출처 적대감사로 검증했다. (`trend_reflection.py`,
   `verify_trends.py`) — **종합 보고서**(분석·발견·역학·검증): [`TRENDS.md`](TRENDS.md)
