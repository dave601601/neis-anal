...
키워드를 손으로 나열하지 않고 FastText 임베딩으로 확장:
- 카테고리 시드의 임베딩 중심 벡터 → 전체 메뉴 어휘에서 코사인 유사도로 관련 메뉴 자동 확장.
- 확장 집합으로 (1) 단기 유행식 시계열·지도, (2) 건강식 vs 유행식 경쟁구도.

산출: figures/refl_*.png + 콘솔(확장된 키워드 포함).
"""
import re
import numpy as np
import pandas as pd
...
# 임베딩 확장 시드
FAD_SEED = ["마라", "마라탕", "로제", "탕후루", "두바이", "약과", "그릭", "요거트", "흑당"]
HEALTH_SEED = ["비건", "식물성", "콩고기", "콩단백", "두유", "대체", "버섯", "영양"]
# 시계열에 개별로 그릴 대표 유행 메뉴(부분문자열)
FAD_ITEMS = ["마라", "마라탕", "로제", "탕후루", "두바이", "약과", "그릭"]
...
def expand(model, seeds, topn=45, thr=0.45, min_count=40):
    """시드 임베딩 중심 벡터로 어휘에서 유사 메뉴 확장."""
    cv = np.mean([model.wv[s] for s in seeds], axis=0)
    cv = cv / (np.linalg.norm(cv) + 1e-9)
    vocab = np.array(model.wv.index_to_key)
    V = model.wv.vectors
    Vn = V / (np.linalg.norm(V, axis=1, keepdims=True) + 1e-9)
    sim = Vn @ cv
    cnt = np.array([model.wv.get_vecattr(t, "count") for t in vocab])
    ok = (cnt >= min_count) & (sim >= thr)
    idx = np.argsort(-sim)
    out = [(vocab[i], round(float(sim[i]), 2), int(cnt[i])) for i in idx if ok[i]][:topn]
    return out
...
def main():
    ...
    fad_exp = expand(model, FAD_SEED)
    heal_exp = expand(model, HEALTH_SEED)
    print("=== 임베딩 확장: 유행식 (시드:", "·".join(FAD_SEED), ") ===")
    print("  " + ", ".join(f"{t}({s})" for t, s, c in fad_exp[:30]))
    print("=== 임베딩 확장: 건강식 (시드:", "·".join(HEALTH_SEED), ") ===")
    print("  " + ", ".join(f"{t}({s})" for t, s, c in heal_exp[:30]))

    fad_kw = [t for t, _, _ in fad_exp]
    heal_kw = [t for t, _, _ in heal_exp]
    ...
    for c, kw in zip(cmap, FAD_ITEMS):
        r_ = rate_year(m, re.escape(kw))
        ax.plot(YEARS, r_.values, "-o", color=c, lw=2, ms=4, label=kw)
    ax.set_xticks(YEARS); ax.set_xlabel("연도"); ax.set_ylabel("천 끼당 등장")
    ax.set_title("단기 유행식의 급식 반영 — 마라만 크게, 간식형 유행은 미미", fontsize=12, color=INK)
    ...
    ax2.plot(YEARS, heal_nat.values, "-o", color=HEAL, lw=2.4, ms=5, label="건강식(비건·식물성·콩…)")
    ax2.set_xticks(YEARS); ax2.set_xlabel("연도"); ax2.set_ylabel("천 끼당 등장")
    ax2.set_title("급식 속 경쟁구도 — 학생 유행식 vs 어른 건강식", fontsize=13, color=INK)
