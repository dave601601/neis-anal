...
# 구체적 2021~25 트렌드 root. 로제·흑당·버블티는 2021에 이미 정착/하락(5년 flat)이라
# '2021~25 유행'이 아니므로 제외(감사 결과 로제가 바스켓을 지배해 성장률을 짓눌렀음).
TREND_ROOTS = ["마라", "탕후루", "두바이", "약과", "그릭", "두쫀쿠", "버터떡"]
# 헬시 플레저/대체식품 root. '대체'(=알레르기 치환식 오탐 93%)·'두유'(만두유린기 오탐+스테이플) 제외.
HEALTH_ROOTS = ["비건", "채식", "식물성", "콩고기", "콩불고기", "곡물불고기", "두부까스", "두부텐더"]
# 스테이플(상시 메뉴) — root를 포함하지 않으면 제외
STOP = ["아이스크림", "마카롱", "도넛", "케잌", "케이크", "사탕", "요거트", "요구르트", "만주",
        "다쿠아즈", "오란다", "쿠키", "젤리", "우유", "프리첼", "파이", "푸딩", "빙수"]
# 개별 라인용 대표 유행(부분문자열)
FAD_ITEMS = ["마라", "탕후루", "두바이", "약과", "그릭"]
...
def main():
    ...
    print(f"수도권 {su:.1f} vs 비수도권 {no:.1f} (차 {su-no:+.1f}/천끼 — 무의미; 판정은 거리상관으로)")
