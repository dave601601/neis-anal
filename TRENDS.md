...
**세 겹으로 검증**했다 — (1) 스크립트 재현, (2) 부트스트랩 재표집, (3) 외부 출처 적대감사(§7).

> **이 문서를 읽는 법.** §1–2는 질문과 방법, §3–5는 데이터에서 나온 세 가지 발견(*무엇이* 일어났나),
> §6은 그 원인의 역학(*왜* 일어났나), §7은 검증 과정이다. 발견 절 끝의 *"→ §6.x"* 포인터를 따라가면
> 곧바로 그 발견의 원인 설명으로 이어진다.
...
### 2.2 키워드를 손으로 안 고르는 이유, 그리고 그 함정
...
> **임베딩에도 함정이 있었다(중요).** 초판은 여러 시드 단어의 *중심 벡터(centroid)* 로 확장했다. 그런데
> *마라(매운맛) + 탕후루(디저트) + 요거트(유제품)* 를 평균한 중심 벡터는 "유행"이 아니라 **"디저트 일반"**
> 쪽을 가리켰다. 그 결과 아이스크림(단독 3.7만 건)·마카롱·도넛 같은 *상시* 디저트가 '유행식' 집계를
> 지배해 버렸다(가짜 신호).
...
**일부러 뺀 단어와 그 이유**(5라운드 적대검증에서 걸러냄, §7.1):
...
- 2025년 기준 **유행식이 건강식의 약 2.9배**다. 다만 *압도*는 아니다 — 초판이 말한 "10배"는 앞서 말한
  디저트 오염(아이스크림 등)이 만든 허수였고, 정제 후엔 2.9배로 내려왔다. 건강식도 ×1.7로 꾸준히 는다
...
대형 유행(마라 CV 0.19)은 **전국이 고르게** 받아들이고, 틈새 유행(비건·바질)만 지역색이 강하다. 이는
§1 공간분석의 '매운맛=영남 핫스팟'과 결이 같다 — 영남·호남이 신메뉴를 더 빨리 받아들인다.
...
세 발견의 *무엇* 은 데이터로 끝났다. *왜* 는 우리 데이터 *밖*(급식 제도·정책·세대)에 있어, 정부 법령·
교육청 자료·학술 논문·언론을 다중 출처로 조사했다. 세 역학은 하나의 축으로 수렴한다 — **급식은 학생
문화의 거울이 아니라, 제도가 미리 짠 캘린더다.** (네 패턴 전체의 교차분석은 [`DYNAMICS.md`](DYNAMICS.md).)
...
> **단서(적대감사 반영).** "건강식 = 순수 top-down"은 약간 과하다. 채식주의자·질환이 있는 학생·학부모가
...
이 분석의 결론은 처음과 많이 다르다("유행:건강 10배 압도", "대구가 트렌드 리더"는 모두 무너졌다).
세 겹으로 검증했다.

### 7.1 5라운드 적대검증 (키워드 표류 → 정직화)
독립 감사자가 5라운드에 걸쳐 구멍을 잡고 고쳤다. 라운드마다 구멍이 작아지며 수렴했다.

| 라운드 | 잡은 구멍 | 수치 변화 |
|---|---|---|
| **초판** | 임베딩 centroid 표류 → 아이스크림·마카롱·도넛이 '유행식' 지배 | 유행:건강 "10배" |
| **R1** | 로제(정착메뉴) 바스켓 지배 · '대체'(치환식 오탐 93%) · 마라탕 이중계수 | → 3.6배 → 2.9배 |
...
세 스크립트를 다시 돌려 모든 핵심 수치가 문서와 일치함을 확인했다: 마라 ×6.9(MK 10·CAGR 62%)·두바이
×21.5·유행:건강 2.9배·광주 +0.46·경북 +0.43·서울거리 r=+0.07(p=0.81). 더 나아가, 문서에 *재현 코드가
없던* 네 수치를 `verify_trends.py`로 직접 재현·교정했다.

| 주장 | 문서(이전) | 재현 결과 | 처리 |
|---|---|---|---|
| 유행 바스켓 내 마라 비중 | "79%" | 78.7%(2025년 79.7%) | 확정 |
| 수도권 둔감 부트스트랩 | "89%" | **90%**(마라 제외 93%) | 89 → 90 교정 |
| 시도 1위 안정성 | "대구 1위 53% / 마라 빼면 대구 4위" | **광주 33%**(마라 제외 40%), 대구 12% | stale 교정(마라탕 제거 전 값) |
| 경쟁 정체 수준 | "유행 48→50, 건강 13.7→13.7" | 유행 28→30(정체), 건강 11.8 → **10.3**(하락) | stale 교정 |

> "재현 코드가 없는 수치"가 가장 위험했다 — '89%'는 어느 스크립트에도 계산이 없었는데, 직접 부트스트랩
> 하니 90%·광주(대구 아님)로 정직화됐다. §5의 부트스트랩 그림 두 장이 그 재현 결과다.

### 7.3 외부 출처 적대감사 (§6 역학 주장)
독립 감사자 둘이 §6의 외부 사실을 *반증하려는* 자세로 원문을 다시 확인했다.

- **제도·재정(§6.1–6.2)**: 6건 모두 CONFIRMED — 별표3 단순당류 문구, 매월 27일 계약, 추가지원 단가
  (경북 1,084·전남 803·서울 0), 전남 차액지원(약 393억), 서울 친환경 70→50%, 특별법 = '판매' 규제.
  과장·왜곡 없음.
- **경쟁구도(§6.3)**: 2011 광주 최초·잔반 30~40%·만족도 33%·탄소중립 102→340교는 1차 출처와 일치.
  **교정 3건 반영** → ① 인천 81% 반등은 '환경교육·텃밭 운영 선도학교 6곳' 값으로 정밀화(출처 유스저널),
  ② '채식선택제 전환'은 서울 2024 등으로 출처 정정(인용 기사가 서울 건이었음), ③ "순수 top-down" 단정은
  *소수 bottom-up*(선택권 요구) 단서를 달아 완화.
...
1. **어른들은 학생 유행을 절반만 따라간다.** 마라처럼 *메뉴화 가능한* 유행은 빠르게(×6.9) 흡수하지만,
   탕후루·두바이초콜릿 같은 *간식·디저트형* 유행은 제도(영양기준·사전편성)가 거른다. 유행 바스켓의 80%가
   마라라, "유행 반영"은 사실상 "마라 반영"이다.
2. **유행식이 건강식을 2.9배 앞서지만 압도하진 않으며, 둘 다 2023 이후 정체한다.** 학생이 미는 유행과
   어른이 미는 건강식은 **서로의 필터에 걸리는 양방향 거울**이다 — 유행은 제도가, 건강식은 학생 입맛이
   거른다.
3. **트렌드 리더는 수도권이 아니라 지방(호남·영남)이다.** "도시=리더" 통념을 데이터가 기각한다(수도권
   둔감 부트스트랩 90%). 도시가 아니라 *지역의 재정 여력*이 트렌드를 만든다.
...
출처는 §6 **역학(why)** 의 제도·정책 사실에 대한 것이며, 적대감사(§7.3)로 원문을 재확인했다.
