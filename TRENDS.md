...
### 5.3 '평균·줄세우기'를 넘어 — 진짜 공간통계 (Moran's I · LISA · Gi*)
위까지는 시도(16)를 평균해 비교한 것이라, 그것만으론 "엑셀 피벗이지 공간분석이 아니다"라는 비판을
받는다 — *인접한 지역끼리 실제로 닮았는지*는 공간통계로 따로 검정해야 한다. 그래서 같은 트렌드 바스켓을
**시군구 187개(≥5교)** 로 내리고, 인접(Queen) 가중치 위에서 공간 자기상관을 검정했다(`trend_spatial.py`).

<img src="assets/g1-trend-lisa.png" width="350" alt="시군구 트렌드 수용 LISA — 영남 대도시 핫스팟">

- **Global Moran's I = +0.11, p = 0.019**(순열 999회) → 트렌드 수용은 **무작위가 아니라 공간적으로
  군집**한다(약하지만 유의). *피벗이 아니라 검정된 공간 구조다.*
- **LISA 핫스팟(HH)** 은 **영남 대도시(부산·대구·울산)** 에 집중, 콜드스팟(LL)은 경남 서부 농촌. Getis-Ord
  Gi*도 같은 그림(핫 16·콜드 7곳). 이 핫스팟은 §1 식문화 분석의 **'매운맛=영남'** 패턴과 겹친다 — 바스켓의
  80%가 마라(매운맛)라 사실상 **마라의 영남 집중을 재확인**한다(과대해석 금지).
- **해상도가 초점을 옮긴다(MAUP)**: 시도(거시)에선 광주·경북이 1위였지만, 시군구(미시)에서 *통계적으로
  유의한* 핫스팟은 영남 대도시다. 단위를 바꾸면 그림이 달라진다는 MAUP를 회피하지 않고 **측정**했다 —
  거시(수도권 둔감, 부트스트랩 90%)와 미시(영남 도시 핫스팟)는 서로 다른 층위의 사실이다.

*왜 수도권은 둔감하고(거시) 영남은 핫한가(미시)? → §6.2 재정 인센티브 + §1 식문화.*
...
3. **트렌드를 먼저 받는 건 도시가 아니다 — 예산이다.** 수도권은 둔감하고(부트스트랩 90%) 영남이
   핫하다. 트렌드 수용은 무작위가 아니라 **공간적으로 군집**하며(Moran's I=+0.11, p=0.02; LISA 핫스팟=
   부산·대구·울산), 그 패턴은 입맛이 트렌디해서가 아니라 친환경 급식 예산이 지방에 더 많아서 생긴다.

> **급식에 뭐가 오를지는 '맛·인기·도시'가 아니라 '형태·제도·예산'이 정한다.** 그래서 우리의 상식이
> 세 번 다 빗나간다.

> **지리적 결론.** 급식 트렌드의 지도는 두 공간 구조가 겹친 것이다 — **정책의 공간**(친환경 예산이
> 수도권 0 vs 지방 고로 양극화)과 **식문화의 공간**(영남 매운맛). 그리고 그 경계가 하필 시도와 겹치는
> 건 우연이 아니다 — **시도는 임의로 그은 분석 단위가 아니라 예산·정책이 집행되는 실제 경계**이기
> 때문이다(MAUP를 약점이 아니라 근거로 뒤집는다).
...
- **공간 단위(MAUP)**: 거시(시도)와 미시(시군구)의 그림이 달라 결과는 단위에 민감하다. 트렌드 바스켓이
  sparse해 시군구 공간자기상관(I=+0.11)은 약하고, 80%가 마라라 핫스팟은 사실상 마라 분포다.
- **예산 데이터 한계**: 17개 시도 전체의 친환경 추가지원 단가는 공개되지 않았다(서울·부산 0·인천 554·
  전남 803·경북 1,084 등 5개 시도만 확인). '수도권 0 vs 지방 고'는 *부분 데이터*에 근거한 방향이다.
  거리 검정도 유클리드 직선거리 한정(교통망·통행시간 미반영).
