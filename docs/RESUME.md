...
## 7. 발표 포스터 — 영어 A1 완성 (2026-06-07)
트렌드 발표용 A1 포스터 → **[`../poster_outline.html`](../poster_outline.html)** (3단·1페이지·전부 영어).
- 프레임 "상식 3개 뒤집기": 인기·학생수요·도시 통념을 **form·institution·space**(맛·인기·도시가 아니라
  형태·제도·예산)로 뒤집음. FastText 방법 강조박스, 유행식/건강식 한반도 choropleth, 마라 LISA(Moran's I=0.25,
  p<0.001) GIS hero + **Moran's I 공식·핫스팟 이유**.
- 그림: `poster_figs_en.py` → `assets/en/` 5장(영어·흰 배경·차분한 팔레트, 신호등 색 금지). 변환:
  weasyprint→pdftoppm→PNG. 마크다운 문서 PDF는 `md_to_pdf.py`.
- 공간통계 v2(`trend_spatial.py`: 마라 단독 Moran's I=0.26 p=0.001, 비마라 비유의 → 트렌드 공간성=마라;
  마라≠매운맛 r=−0.18 도시 외식)는 시각자료 입력 적대평가 2라운드 통과.

## 8. ▶ 다음 작업 (NEXT)
- 포스터 **팀명/멤버 placeholder를 실제 이름으로** (poster_outline.html 헤더).
- (선택) DYNAMICS ③(전통 후퇴·마라=영남) 적대검증. (구) `poster.html`(시도 한글 공간 포스터)는 별개 보존.
