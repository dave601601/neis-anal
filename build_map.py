...
<div class="wrap">
  ...
  <div class="foot">
    수치는 시도별 학교 평균. 점유율은 한 끼 메뉴 중 해당 형태 비율, 사용도는 단백질 속성 합, 유사도는 학교 임베딩과 키워드 벡터의 코사인 유사도를 지역 비교용으로 z-표준화한 값(0=전국평균). 군집은 KMeans 결과.
  </div>
</div>
...
const METRICS = [
  ...
  {key:'sim_seafood',   label:'해산물 유사도',     type:'div'},
  {key:'sim_protein',   label:'단백질 유사도',     type:'div'},
  {key:'sim_rice',      label:'밥 유사도',         type:'div'},
  {key:'sim_bread',     label:'빵 유사도',         type:'div'},
  {key:'sim_noodle',    label:'면 유사도',         type:'div'},
...
function showPanel(sido){
  ...
  const rows=[
    ...
    ['해산물 유사도(z)',d.sim_seafood.toFixed(2)],
    ['단백질 유사도(z)',d.sim_protein.toFixed(2)],
  ];
...
METRICS.forEach((m,i)=>{
  ...
  b.onclick=()=>{current=m;[...cbox.children].forEach(c=>c.classList.remove('active'));b.classList.add('active');
    document.getElementById('hint').textContent = m.type==='div'?'임베딩 키워드 유사도 · z-표준화(전국평균 0 기준 상대값)':(m.type==='cat'?'시도별 우세 군집(KMeans)':'한 끼 메뉴 중 비율(시도 평균)');
    render();};
  cbox.appendChild(b);
});
